...
import (
    ...
    apimodel "github.com/Dysar/url-crawler/backend/internal/models/api"
...
func (h *URLHandlers) CreateURL(c *gin.Context) {
    var req apimodel.CreateURLRequest
    ...
    c.JSON(http.StatusCreated, gin.H{
        "data": apimodel.URLResponse{ID: rec.ID, URL: rec.URL},
        "error": nil,
        "message": "Success",
    })
}
...
func (h *URLHandlers) ListURLs(c *gin.Context) {
    ...
    resp := make([]apimodel.URLResponse, 0, len(rows))
    for _, r := range rows {
        resp = append(resp, apimodel.URLResponse{ID: r.ID, URL: r.URL})
    }
