...
func (s *JobService) process(ctx context.Context, jobID int64, urlID int64, target string) {
    _ = s.jobs.UpdateStatus(ctx, jobID, dbmodel.JobRunning, nil)
    res, err := s.craw.Crawl(ctx, target)
    if err != nil {
        msg := err.Error()
        _ = s.jobs.UpdateStatus(ctx, jobID, dbmodel.JobFailed, &msg)
        return
    }
    // Persist
    ...
    title = res.Title
    _, _ = s.results.Create(ctx, dbmodel.CrawlResult{
        JobID: jobID,
        URLID: urlID,
        HTMLVersion: htmlVer,
        Title: title,
    ...
    _ = s.jobs.UpdateStatus(ctx, jobID, dbmodel.JobCompleted, nil)
}
